...
public class Question2_retry {
    public int solution(int[] topping) {
        ...
        for(int i=0;i<topping.length;i++){
            if(!hMap1.isEmpty() && hMap1.get(topping[i])!=null) {
                hMap1.put(topping[i], hMap1.get(topping[i])-1);
                hMap2.put(topping[i], hMap2.getOrDefault(topping[i], 0)+1);
            }

            if(hMap1.get(topping[i])==0) hMap1.remove(topping[i]);
    ...
    public static void main(String arg[]){
        Question2_retry s = new Question2_retry();
